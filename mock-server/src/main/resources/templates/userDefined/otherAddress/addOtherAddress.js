layui.use([
	...
	form.verify({
		...
		url : function(val) {
			return checkInputLength(val, null, 50, "访问地址URL")
		},
	...
	form.on("submit(addOtherAddressSubmit)", function(data) {
		// 弹出loading
		var index = top.layer.msg('数据提交中，请稍候', {
			icon : 16, time : false, shade : 0.8
		});

		var postData = $("#addOtherAddressForm").serialize();

		// 实际使用时的提交信息
		$.post("/otherAddress/addOtherAddress", postData, function(response) {
